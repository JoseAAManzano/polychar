...
def calculate_divergence(words, trie, model, vectorizer=None):
    ...
    for i, word in enumerate(words):
        if isinstance(model, utils.CharNGram):
            m_dist = np.float32(list(model.get_distribution_from_context(word[:-1]).values()))
        else:
            m_dist = get_distribution_from_context(model, word[:-1], vectorizer)
...
results_data = results[['model', 'prob', 'data', 'run',
                        'accuracy_train','perplexity_train', 'accuracy', 'perplexity']]
                        # 'KL_train','KL_test']]

results_acc = pd.melt(results_data, id_vars=['model', 'prob', 'data', 'run'],
                      value_vars=['accuracy_train', 'accuracy'], var_name='split',
                      value_name='acc')
results_acc['split'] = np.where(results_acc.split == 'accuracy_train', 'train', 'test')
...
results_perp = pd.melt(results_data, id_vars=['model', 'prob', 'data', 'run'],
                      value_vars=['perplexity_train', 'perplexity'], var_name='split',
                      value_name='perp')
results_perp['split'] = np.where(results_perp.split == 'perplexity_train', 'train', 'test')
...
for data, category in zip(args.datafiles, args.modelfiles):
    for prob in args.probs:
        ...
        train_words = list(dataset.train_df.data)
        trie_train = utils.Trie().insert_many([list(w) + '</s>' for w in train_words])
        
        test_words = list(dataset.test_df.data)
        trie_test = utils.Trie().insert_many([list(w) + '</s>' for w in test_words])
...
for data, category in zip(args.datafiles, args.modelfiles):
    for prob in [40, 50, 60, 99]:
        ...
        for run in range(args.n_runs):
            ...
            loss_fn = nn.CrossEntropyLoss(ignore_index=vectorizer.data_vocab.PAD_idx)
            optim = torch.optim.Adam(lstm_model.fc.parameters(), lr=args.learning_rate)
            for it in range(5):
